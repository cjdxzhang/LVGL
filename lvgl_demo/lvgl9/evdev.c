...
int evdev_get_tp_event(void)
{
    ...
    {
        LV_LOG_WARN("%s: open %s filed", __func__, path);
        return -1;
    }
    ...
    {
        int idx = -1;
        if (sscanf(ent->d_name, "input%d", &idx) != 1 || idx < 0)
            continue;

        sprintf(file_name, "/sys/class/input/input%d/name", idx);
        fd = open(file_name, O_RDONLY);
        if (fd == -1)
        {
            LV_LOG_WARN("%s: open %s failed", __func__, file_name);
            continue;
        }
        ...
        if (len <= 0)
        {
            LV_LOG_WARN("%s: read %s failed", __func__, file_name);
            continue;
        }
        ...
        {
            lsensor_event_id = idx;
            continue;
        }

        if (strstr(tp_name, "proximity"))
        {
            psensor_event_id = idx;
            continue;
        }
#else
        if (strstr(tp_name, "lightsensor"))
        ...
        if (strstr(tp_name, "ts") || strstr(tp_name, "gsl") || strstr(tp_name, "Goodix") || strstr(tp_name, "goodix"))
        {
            sprintf(tp_event, "/dev/input/event%d", idx);
            LV_LOG_USER("%s: %s = %s%s", __func__, file_name, tp_name, tp_event);
            closedir(pDir);
            return 0;
        }
    }
...
int evdev_init_psensor(void)
{
    ...
    if (psensor_event_id == -1)
    {
        ...
        {
            LV_LOG_WARN("%s get event failed", __func__);
            return -1;
        }
        if (psensor_event_id == -1)
        {
            LV_LOG_WARN("%s get psensor event failed", __func__);
            return -1;
        }
    }
    ...
    if (fd < 0)
    {
        LV_LOG_WARN("can't open /dev/psensor!");
        return -1;
    }
    ...
    if (ret < 0)
    {
        LV_LOG_WARN("enable /dev/psensor failed %d", ret);
    }
    else
    {
        LV_LOG_USER("enable /dev/psensor successfully");
    }
    close(fd);
    ...
    if (fd < 0)
    {
        LV_LOG_WARN("can't open %s", event_name);
        return -1;
    }
    psensor_fd = fd;
...
int evdev_init_lsensor(void)
{
    ...
    if (lsensor_event_id == -1)
    {
        ...
        if (lsensor_event_id == -1)
        {
            LV_LOG_WARN("%s get lsensor event failed", __func__);
            return -1;
        }
    }
    ...
    if (fd < 0)
    {
        LV_LOG_WARN("can't open /dev/lightsensor!");
        return -1;
    }
    ...
    if (ret < 0)
    {
        LV_LOG_WARN("enable /dev/lightsensor failed %d", ret);
    }
    else
    {
        LV_LOG_USER("enable /dev/lightsensor successfully");
    }
    close(fd);
...
int evdev_init(lv_display_t *disp, int rot)
{
    ...
    {
        LV_LOG_WARN("%s get tp event failed", __func__);
        return -1;
    }
...
int evdev_set_file(lv_display_t *disp, char *dev_name)
{
    ...
    if (rc < 0)
    {
        LV_LOG_WARN("Failed to init libevdev (%s)", strerror(-rc));
    }
    else
    {
        if (libevdev_has_event_type(evdev, EV_ABS))
        {
            ...
            if (libevdev_has_event_code(evdev, EV_ABS, ABS_MT_POSITION_X))
            {
                ...
                LV_LOG_USER("EV_ABS ABS_MT_POSITION_X min=%d max=%d", abs->minimum, abs->maximum);
            ...
            if (libevdev_has_event_code(evdev, EV_ABS, ABS_MT_POSITION_Y))
            {
                ...
                LV_LOG_USER("EV_ABS ABS_MT_POSITION_Y min=%d max=%d", abs->minimum, abs->maximum);
                if ((evdev_rot == 0) || (evdev_rot == 180))
                {
                    evdev_min_y = abs->minimum;
                    evdev_max_y = abs->maximum;
            ...
            (evdev_max_y != disp_ver))
    {
        ...
        {
            evdev_calibrate = 1;
            LV_LOG_USER("calibrate [%d,%d]x[%d,%d] to %dx%d",
        ...
        else
        {
            ...
            LV_LOG_USER("crop [%d,%d]x[%d,%d] to [%d,%d]x[%d,%d]",
                   evdev_min_x, evdev_max_x,
                   evdev_min_y, evdev_max_y,
                   crop_x1, crop_y1, crop_x2, crop_y2);
        }
    }
    LV_LOG_USER("evdev_calibrate = %d", evdev_calibrate);
