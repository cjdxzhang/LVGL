...
#define WIFI_PERSIST_CONF "/oem/cfg/wpa_supplicant.conf"
#define WIFI_DEFAULT_CONF "/etc/wpa_supplicant.conf"
...
static void wifi_prepare_persist_conf(void)
{
    if (access(WIFI_PERSIST_CONF, F_OK) == 0) {
        return;
    }

    if (system("mkdir -p /oem/cfg && cp " WIFI_DEFAULT_CONF " " WIFI_PERSIST_CONF)) {
        log("Copy wpa_supplicant failed");
    }
}

static void wifi_enable_with_persist_conf(const char *reason)
{
    RK_wifi_register_callback(rk_wifi_state_callback);
    log("enable wifi with %s, reason=%s", WIFI_PERSIST_CONF, reason);
    if (RK_wifi_enable(1, WIFI_PERSIST_CONF) < 0) {
...
static void *wifi_server(void *arg)
{
    ...
    wifi_prepare_persist_conf();
    wifi_enable_with_persist_conf("boot");
    ...
    while (1)
    {
        ...
        switch (cmdarg->cmd) {
            case WIFI_ENABLE:
                log("WIFI_ENABLE\n");
                wifi_enable_with_persist_conf("user");
                break;
            case WIFI_DISABLE:
                log("WIFI_DISABLE\n");
