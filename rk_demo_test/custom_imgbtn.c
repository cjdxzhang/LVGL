...
int16_t to_preparing_flat=1; /* 记录转到preparing的标志 1--注水中，2--自清洁中，3--移动中，4--返回基站中*/
int16_t to_setting_flat=1; /* 记录转到setting的标志 1--从index跳转，2--从working跳转 */
int16_t to_location_flat=1; /* 记录转到location的标志 1--从index跳转，2--从working跳转 3--从preparing跳转 */
...
static void stop_dialog_start_countdown(void)
{
    ...
    lv_obj_add_event_cb(guider_ui.preparing_btn_yes, stop_dialog_button_event_cb, LV_EVENT_CLICKED, NULL);
    lv_obj_add_event_cb(guider_ui.preparing_btn_no, stop_dialog_button_event_cb, LV_EVENT_CLICKED, NULL);
...
static void message_dialog_btn_clicked(lv_event_t *e)
{
    ...
    if (dialog != NULL && lv_obj_is_valid(dialog)) {
        lv_obj_del(dialog);
    }
}
...
void show_message_dialog(lv_obj_t *parent, const char *message)
{
    ...
    lv_obj_set_style_text_font(label, &lv_font_SourceHanSansSC_Regular_25, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_font(btn_label, &lv_font_SourceHanSansSC_Regular_25, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(btn_label, lv_color_hex(0xffffff), LV_PART_MAIN | LV_STATE_DEFAULT);

    /* 单击"确定" → 销毁整个对话框 */
    lv_obj_add_event_cb(btn, message_dialog_btn_clicked, LV_EVENT_CLICKED, dialog);
}
