#include "robot_tcp.h"
#include "lvgl.h" // 用于日志或UI通知，根据实际情况调整
#include <errno.h>
...
static void log_tcp_payload(const char *direction, const char *data, int len)
{
    ...
    //printf("[%s] len=%d payload=%.*s\n", direction, len, len, data);
    fflush(stdout);
}
...
static int send_data(int sock, const char *data, int len)
{
    ...
    if (sent <= 0)
    {
        fprintf(stderr, "[TCP SEND] send failed: %s\n", strerror(errno));
        return sent;
    }
    ...
    if (end_sent <= 0)
    {
        fprintf(stderr, "[TCP SEND] terminator send failed: %s\n", strerror(errno));
        return -1;
    }

    printf("[TCP SEND] terminator=\\r\\n\\r\\n\n");
    fflush(stdout);
...
static int recv_packet(int sock, char **buffer)
{
    ...
    if (ret < 0)
    {
        ...
        //fprintf(stderr, "[TCP RECV] failed to read length header: %s\n", strerror(errno));
        return -1;
    }
    if (ret == 0)
    {
        printf("[TCP RECV] peer closed connection\n");
        fflush(stdout);
        *buffer = NULL;
        return 0;
    }
    ...
    {
        fprintf(stderr, "[TCP RECV] invalid packet length: %u\n", packet_len);
        return -1;
    }
    ...
    while (total_read < (int)packet_len)
    {
        ...
        if (ret <= 0)
        {
            if (ret == 0)
            {
                fprintf(stderr, "[TCP RECV] peer closed during packet read\n");
            }
            else if (errno != EAGAIN && errno != EWOULDBLOCK)
            {
                fprintf(stderr, "[TCP RECV] recv failed: %s\n", strerror(errno));
            }
            free(full_buffer);
            *buffer = NULL;
            return (ret == 0) ? 0 : -1;
...
{
    if (g_work_thread) return -1;
    if (pthread_create(&g_work_thread, NULL, robot_tcp_task, NULL) != 0)
    {
        perror("创建机器人线程失败");
        return -1;
    }
...
int RobotTcp_GotoStation(int station_index)
{
    ...
    if (root)
    {
        ...
        if (cJSON_IsArray(stations) && station_index < cJSON_GetArraySize(stations))
        {
            ...
            if (x && y && z)
            {
                printf("导航至站点 %d: (%.2f, %.2f, %.2f)\n",
                       station_index, x->valuedouble, y->valuedouble, z->valuedouble);
...
int RobotTcp_GotoStationByName(const char *station_name)
{
    ...
    {
        pthread_mutex_unlock(&g_stations_mutex);
        printf("没有可用的站点缓存，请先调用 RobotTcp_GetStationList\n");
        return -1;
    }
