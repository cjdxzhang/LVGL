...
static int submit_command(robot_chassis_command_t *command, robot_station_submit_t submit, void *user_data)
{
    int result;
    if (submit == NULL) {
...
int robot_station_navigate(robot_station_store_t *store, const char *name, robot_station_submit_t submit, void *user_data)
{
    ...
    result = robot_chassis_build_goto(station.x, station.y, station.z, ROBOT_STATION_GOTO_TOLERANCE, &command);
