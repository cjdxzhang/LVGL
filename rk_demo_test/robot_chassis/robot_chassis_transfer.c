    ...
    robot_chassis_backup_transfer_t *transfer, const char *json, size_t length)
{
    ...
    if (step == 1U) {
        ...
        memcpy(transfer->expected_md5, md5->valuestring, 33U);
    } else if (transfer->step_total != step_total ||
               strcasecmp(transfer->expected_md5, md5->valuestring) != 0) {
        result = ROBOT_CHASSIS_ERR_FORMAT;
        goto failed;
    }
    ...
    if (transfer->received_size != transfer->expected_size ||
        fflush(transfer->file) != 0 || fclose(transfer->file) != 0) {
        transfer->file = NULL;
    ...
    transfer->file = NULL;
    {
        ...
        result = calculate_file_md5(transfer->temporary_path, actual_md5);
        if (result != ROBOT_CHASSIS_OK ||
            strcasecmp(actual_md5, transfer->expected_md5) != 0) {
            result = ROBOT_CHASSIS_ERR_CHECKSUM;
            goto failed;
        }
    }
