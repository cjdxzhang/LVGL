...
static int send_all(int fd, const uint8_t *data, size_t length,
                    robot_chassis_send_fn_t send_fn, void *user_data,
                                      ...
                                      const char *json, size_t length)
{
    ...
    result = send_all(client->fd, request, request_length, system_send, NULL, true);
    free(request);
    return result;
}
...
int robot_chassis_client_send_all_for_test(int fd, const uint8_t *data,
                                           ...
                                           void *user_data)
{
    return send_all(fd, data, length, send_fn, user_data, false);
}
