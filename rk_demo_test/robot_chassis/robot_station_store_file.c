...
#define ROBOT_STATION_FILE_VERSION 1
...
int robot_station_store_load(robot_station_store_t *store)
{
    ...
    if (parse_end != text + length || !cJSON_IsObject(root) ||
        !cJSON_IsNumber(version) || version->valuedouble !=
            (double)ROBOT_STATION_FILE_VERSION || !cJSON_IsArray(array)) {
...
static cJSON *build_json(const robot_station_t *stations, size_t count)
{
    ...
    if (root == NULL || array == NULL ||
        !cJSON_AddNumberToObject(root, "version", ROBOT_STATION_FILE_VERSION)) {
        cJSON_Delete(root);
        cJSON_Delete(array);
        return NULL;
    }
...
{
    if (store != NULL) {
        store->test_io_failure_point = (int)failure;
    }
}
