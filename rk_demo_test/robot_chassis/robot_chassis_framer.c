...
static int emit_complete_frames(robot_chassis_framer_t *framer,
                                ...
                                void *user_data, size_t *frame_count)
{
                              ...
                              void *user_data, size_t *frame_count)
{
    ...
    *frame_count = 0U;
    ...
    while (offset < length) {
        ...
        if (copy_length == 0U) {
            result = emit_complete_frames(framer, callback, user_data, frame_count);
            if (result != ROBOT_CHASSIS_OK) {
                return result;
            }
    ...
    return emit_complete_frames(framer, callback, user_data, frame_count);
}
